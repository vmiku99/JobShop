...
void PPPaint(int* NormalGeneChain)
{
	...
	initgraph(1.4 * size, M * 50 + 80, SHOWCONSOLE);
	char g = 'M';
	TCHAR b[6];
	TCHAR T[6];
	setbkcolor(WHITE);
		...
		if (MouseHit())
		{
			...
			if (m.x <= 0.05*size + 38 && m.x >= 0.05*size - 4 && m.y >= 16 && m.y <= 40 && m.uMsg == WM_LBUTTONDOWN)
			{
				...
				if (tt[Maaa][TTime] == 0)
				{
					...
					if (TTime > TT->start)
					{
						for (PP = TT; TT->next != NULL; TT = TT->next)
						{
				...
				else
				{
					...
					{
						if (TT->stop >= TTime||TT->next->next==NULL)
						{
				...
				for (j = 1; j <= ProcedureTotal; j++)
				{
					...
					for (TT = result[ma]; timeStart == -1&&TT->next != NULL && TT->next->next != NULL; TT = TT->next)
					{
	...
	setcolor(BLACK);
	outtextxy(0.1 * size + 40 + Size - 1 - 48, M * 50 + 12 + 12 + 12, _T("Time:"));
	_stprintf_s(b, _T("%d"), Size);
	outtextxy(0.1 * size + 40 + Size - 1 - 8, M * 50 + 12 + 12 + 12, b);
	saveimage(_T("output2.bmp"), NULL);
	_getch();
	_getch();
	closegraph();
	for (i = 0; i <= M; i++)
